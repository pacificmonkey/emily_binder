...
BEGIN
  ...
  WHERE delta > 0 AND created_at >= date_trunc('week', now());

  SELECT COUNT(*) INTO v_active_streaks
  FROM streak_state WHERE status = 'ongoing';

  SELECT COUNT(*) INTO v_tasks_completed_today
  FROM task_instance WHERE canonical_date = CURRENT_DATE AND completion_status = 'done';

  SELECT COUNT(*) INTO v_tasks_today
  FROM task_instance WHERE canonical_date = CURRENT_DATE;
